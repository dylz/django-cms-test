...
class BasicToolbar(CMSToolbar):
    ...
    def add_admin_menu(self):
        ...
        admin_menu = self.toolbar.get_or_create_menu(ADMIN_MENU_IDENTIFIER, user_name)
        if self.request.user.has_perm('user.change_user') and User in admin.site._registry:
            admin_menu.add_modal_item(_('Users'), url=reverse(
        ...
        if len(sites_queryset) > 1:
            ...
            sites_menu.add_modal_item(_('Admin Sites'), url=reverse('admin:sites_site_changelist'))
            sites_menu.add_break(ADMIN_SITES_BREAK)
            for site in sites_queryset:
                sites_menu.add_link_item(site.name, url='http://%s' % site.domain,
                                         active=site.pk == self.current_site.pk)
                # admin
        admin_menu.add_modal_item(_('Administration'), url=reverse('admin:index'))
        admin_menu.add_break(ADMINISTRATION_BREAK)
        # cms users
        #admin_menu.add_modal_item(_('User settings'), url=reverse('admin:cms_usersettings_change'))
...
class PageToolbar(CMSToolbar):
    ...
    def change_admin_menu(self):
        ...
        admin_menu.add_modal_item(_('Pages'), url=url, position=0)
